-- FILENAME: CGD/Gravity/RealityFilters/TypeN_NilpotentPart2_4.lean

import CGD.Gravity.RealityFilters.TypeN_NilpotentPart2_3
...
/-- Transverse spatial derivatives of a purely retarded-time dependent function f(t-z). -/
lemma partialDeriv_retarded_k
  (f : ℝ → ℂ) (k : Fin 4) (x : SpacetimePoint) (hk0 : k ≠ 0) (hk3 : k ≠ 3)
  (hf : DifferentiableAt ℝ f (x 0 - x 3)) :
  partialDeriv k (fun p => f (p 0 - p 3)) x = 0 := by
  unfold partialDeriv
  ...
  have hL_val : L (Pi.single k 1) = 0 := by
    -- We use `change` to definitionally evaluate the ContinuousLinearMap application
    change (Pi.single k (1:ℝ) : SpacetimePoint) 0 - (Pi.single k (1:ℝ) : SpacetimePoint) 3 = 0
    have h0 : (Pi.single k (1:ℝ) : SpacetimePoint) 0 = 0 := by simp [hk0]
    have h3 : (Pi.single k (1:ℝ) : SpacetimePoint) 3 = 0 := by simp [hk3]
    rw [h0, h3, sub_zero]
    
  rw [hL_val]
  exact ContinuousLinearMap.map_zero (fderiv ℝ f (x 0 - x 3))
