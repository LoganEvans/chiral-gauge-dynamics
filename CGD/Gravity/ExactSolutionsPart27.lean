-- FILENAME: CGD/Gravity/ExactSolutionsPart27.lean

import CGD.Gravity.ExactSolutionsPart26
...
theorem dynamicExactLorentzianSolution :
  ...
  have h_curv : ∀ m n, curvatureSl2c u.sd_sector m n origin = urb_F_origin m n := by
    ...
    have h_sd_full : u.sd_sector = ⟨exactLorentzianField, exactLorentzian_smooth⟩ := by
      ...
      exact h_sd_val
    rw [h_sd_full]
    rw [curvature_origin_eq m n]
    unfold urb_F_origin
    rw [c_F_mat_eval m n]

  constructor
  · simp only [h_cgd_adj]
    exact CDJ_constraint_holds
  · have h_metric_eq : (fun m n => curvatureSl2c u.sd_sector m n origin) = urb_F_origin := by
      funext m n
      exact h_curv m n
    rw [h_metric_eq]
    exact metric_is_Lorentzian
