...
lemma urbantke_nondeg_of_plebanski 
  ...
  rw [h_cgd_det_zero] at h_det_g_adapter
  
  have h_lam_zero : Λ = 0 := by
    have h_eq : 0 * (-9 / 4 : ℂ) = (-729 / 256 : ℂ) * Λ^6 := by
      calc 0 * (-9 / 4 : ℂ) = Matrix.det (g ()) * (-9 / 4 : ℂ) := by rw [h_det_g_adapter]
      _ = Matrix.det (g ()) * (eta ())^2 := h_left.symm
      _ = (Matrix.det (invPsi_3x3 ()))^2 := h_det_id
      _ = (-729 / 256 : ℂ) * Λ^6 := h_right
    have h_eq2 : Λ^6 = 0 := by
      calc Λ^6 = (-256 / 729 : ℂ) * ((-729 / 256 : ℂ) * Λ^6) := by ring
      _ = (-256 / 729 : ℂ) * (0 * (-9 / 4 : ℂ)) := by rw [← h_eq]
      _ = 0 := by ring
