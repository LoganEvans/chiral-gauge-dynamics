...
section BornRuleEquivalence

variable (x : ℝ)

@[litlib_track "Geometric Phase Space Fraction evaluates to the Born Rule - Fraction"]
lemma hopfVolumeIsBornRuleFraction (h_lower : -1 ≤ x) (h_upper : x ≤ 1) :
  let theta := Real.arccos x;
  hopfPhaseSpaceFraction theta = (1 + x) / 2 := by
  intro theta
...
@[litlib_track "Geometric Phase Space Fraction evaluates to the Born Rule - Cosine"]
lemma hopfVolumeIsBornRuleCos (h_lower : -1 ≤ x) (h_upper : x ≤ 1) :
  let theta := Real.arccos x;
