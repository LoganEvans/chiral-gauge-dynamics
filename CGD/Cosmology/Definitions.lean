...
def isIsotropic (u : Universe) (f : SpacetimePoint → Complex) : Prop :=
  (∀ x i j, i ≠ 0 → j ≠ 0 → curvatureSl2c u.self_dual i j x = (f x) • spatialIsotropicAnsatz i j) ∧
  (∀ x i, curvatureSl2c u.self_dual i i x = 0)
...
def isStaticUniverse (u : Universe) : Prop :=
  ∀ x j, curvatureSl2c u.self_dual 0 j x = 0
