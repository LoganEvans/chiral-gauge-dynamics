...
theorem kinematicBigBang (u : Universe)
  ...
  (h_non_degenerate : ∀ x, x 0 = 0 → urbantkeMetric (fun mu nu => curvatureSl2c u.light mu nu x) ≠ 0) :
  ∀ x, x 0 = 0 → ∃ c : Complex, c ≠ 0 ∧ urbantkeMetric (fun mu nu => curvatureSl2c u.light mu nu x) = c • 1 := by
  intro x hx
  have h_symm := h_tic x hx
  have h_g := urbantke_eq_smul_id_of_self_dual (fun mu nu => curvatureSl2c u.light mu nu x) h_symm
...
theorem kinematicStaticUniverseDegeneracy (u : Universe) :
  isStaticUniverse u →
  ∀ x, (urbantkeMetric (fun mu nu => curvatureSl2c u.light mu nu x)).det = 0 := by
  intro h x
  have h_static : ∀ j : Fin 4, curvatureSl2c u.light 0 j x = 0 := h x
  have h_metric_zero_row := math_Static_Electric_Zero (fun mu nu => curvatureSl2c u.light mu nu x) h_static
