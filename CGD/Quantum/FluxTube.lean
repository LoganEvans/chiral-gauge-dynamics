...
theorem kinematicFluxTubeStability (u : Universe) :
  ∀ (x : SpacetimePoint),
    isFluxTube u.light x →
    (urbantkeMetric (fun m n => curvatureSl2c u.light m n x)).det = 0 := by
  intros x h_flux; unfold isFluxTube at h_flux; rcases h_flux with ⟨h_val, h_deriv⟩
  have h_curv : ∀ mu nu, curvatureSl2c u.light mu nu x = curvatureSl2c fluxTubeFrame mu nu x :=
    curvature_congruence u.light fluxTubeFrame x h_val h_deriv
  have h_F : (fun m n => curvatureSl2c u.light m n x) = (fun m n => curvatureSl2c fluxTubeFrame m n x) := by funext m n; exact h_curv m n
