...
In this section, we will start by characterizing the CGD condensate and exploring the phenomenological implications related to the proton radius puzzle.

To evaluate the proton radius, we first consider the empty vacuum where the anti-self-dual (ASD) sector's curvature is zero. If the self-dual (SD) sector were structurally equivalent to the ASD sector, then both sectors would be zero and thus the field would trivially commute with itself. However, if the field trivially commuted with itself, then Equation \ref{eq:kinematicSingleColorDegeneracy} shows that the universe would lack volume, which would contradict the CGD Axiom \ref{eq:macroscopicVolumeAxiom} asserting that the universe has volume. Thus, we can show that the universe must have chirality

\begin{equation}
    \left( \forall \mu, \nu, \ F^{(R)}_{\mu\nu}(x) = 0 \right) \implies \mathcal{A}^{(L)} \neq \mathcal{A}^{(R)}.
    \quad \leanref{CGD.Phenomenology.Chirality.macroscopicVolumeImpliesChirality}
...
We can further characterize the condensate by observing that both the SD and ASD Lie algebras are restricted to \slTwoCAlg, so they are traceless matrices by definition. Since the trace is a linear operation, we conclude that the condensate is isovector

\begin{equation}
    \mathrm{Tr}(\mathcal{A}^{\mathrm{axial}}_\mu(x)) = 0 .
    \quad \leanref{CGD.Phenomenology.AxialCondensate.axialIsIsovector}
...
\begin{equation}
    ...
    \quad \leanref{CGD.Phenomenology.AxialCondensate.axialIsParityOdd}
