...
In an ideal world, all of the physics literature we rely upon would be fully formalized from basic mathematical axioms, but that does not exist, so we created the Litlib library of transcriptions from published literature into Lean 4 code. The Lean 4 environment treats these transcriptions as axiomatically true, so we have to first trust that the papers contain no errors, and then we must trust that the transcriptions into Lean 4 are accurate. While this solution is not perfect, it is the best solution we could engineer because it isolates the unformalized portions of the math.

This paper includes the formalized Lean 4 signatures in Appendix \ref{sec:code_summary}. In order to save space, this summary contains only the theorem signatures and the definitions required to understand those signatures. Many of the theorems we show here contain a For the complete, compilable proofs, please refer to the Supplemental Materials. Within the main body of this paper, we use mathematical notation to write the equations, but we include a specialized footnote reference that matches to the Lean 4 code name in the Code Summary index and also contains a hyperlink navigational aid to allow readers to quickly jump to the formalized definition. For example, in \mbox{$\slashed{D}_\nu = e_a^\mu \gamma^a \left( F_{\mu\nu} + \partial_\nu A_\mu \right) \leanref{CGD.Quantum.generalizedDynamicDiracEquation}$}, the \leanref{CGD.Quantum.generalizedDynamicDiracEquation} matches the \texttt{CGD.Quantum.generalizedDynamicDiracEquation} entry in the Code Summary.
...
In the appendices, we briefly summarize other proofs found within the Lean 4 code.
