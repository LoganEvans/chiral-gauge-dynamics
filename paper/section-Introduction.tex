...
This paper includes the formalized lean 4 signatures in Appendix \ref{sec:code_summary}. In order to save space, this summary contains only the theorem signatures and the definitions required to understand those signatures. Many of the theorems we show here contain a For the complete, compilable proofs, please refer to the Supplimental Materials. Within the main body of this paper, we use mathematical notation to write the equations, but we include a specialized footnote reference that matches to the lean 4 code name in the Code Summary index and also contains a hyperlink navigational aid to allow readers to quickly jump to the formalized definition. For example, in \mbox{$\slashed{D}_\nu = e_a^\mu \gamma^a \left( F_{\mu\nu} + \partial_\nu A_\mu \right) \leanref{CGD.Quantum.generalizedDynamicDiracEquation}$}, the \leanref{CGD.Quantum.generalizedDynamicDiracEquation} matches the \texttt{CGD.Quantum.generalizedDynamicDiracEquation} entry in the Code Summary.
