...
\todo{WTF?}
CGD mathematically allows for particles with properties consistent with candidates for dark matter. We used the Abelian projection to define the eletric current in Equation \ref{eq:kinematicChargeConservation}, but if we instead evaluate a non-Abelian topological defect $([F_{\mu\nu}, F){\rho\sigma}] \ne 0)$, we end up with a defect that possesses zero electric charge.

\begin{equation}
    [F_{\mu\nu}, F_{\rho\sigma}] \neq 0 \implies m_{\text{inertial}} > 0 \;\land\; \text{Tr}([F, F][F, F]) \neq 0 \quad \leanref{CGD.Cosmology.emergentDarkMatterProfile}
\end{equation}
