...
\subsection{The Proton Radius Puzzle}
...
\todo{
\textbf{Author Prompt (The Boer-Mulders Effect):}
\begin{itemize}
    \item Introduce the Boer-Mulders effect (the correlation between quark transverse spin and transverse momentum).
    \item Explain that because this observable projects the orthogonal SU(2) topology (via $\sigma_z$ instead of $\sigma_x$), it obeys the exact same geometric sign-flip mechanism as the Sivers effect.
\end{itemize}
}

\begin{equation}
    \langle \text{Kick}_{\text{BM}}(L) \rangle = - \langle \text{Kick}_{\text{BM}}(-L) \rangle
    \quad \leanref{CGD.Phenomenology.kinematicBoerMuldersSignFlip}
    \label{eq:kinematicBoerMuldersSignFlip}
\end{equation}

\todo{
\textbf{Author Prompt (The TMD Geometric Ratio - The Bombshell):}
\begin{itemize}
    \item Highlight that standard QCD treats Sivers and Boer-Mulders as independent empirical functions.
    ...
    \item Emphasize that predicting a flat kinematic ratio ($\frac{\text{BM}}{\text{Sivers}} = -\tan(\alpha)$) is a highly restrictive, testable, and falsifiable departure from standard phenomenological fits.
\end{itemize}
}

\begin{equation}
    \cos(\alpha) \langle \text{Kick}_{\text{BM}} \rangle = - \sin(\alpha) \langle \text{Kick}_{\text{Sivers}} \rangle
