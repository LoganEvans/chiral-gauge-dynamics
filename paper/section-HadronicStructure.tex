...
The prediction of an axial condensate also has implications for transverse momentum-dependent distributions (TMD). These are effects where the internal structure of a proton is measured by bombarding the hadron with an electron and measuring the byproducts. Eight total TMDs exist, but three of them (Sivers, Boer-Mulders, and the Worm-Gear) are simple enough mathematically that we can explore them analytically.

The Sivers effect is a phenomenon where the transverse momentum of quarks depends on the spin of the proton. We will conjecture that a flux tube (Equation \ref{eq:fluxTubeFrame}) connects the internal topological poles (aka quarks). By evaluating the Wilson line, we find that the path integral collapses into a matrix exponential. This implies that reversing the limits of integration will invert the sign of the transverse momentum kick:
\begin{equation}
...
When we model the Boer-Mulders effect, which measures the momentum kick of a spinning quark instead of the spinning proton in the Sivers Effect, we find that the model produces the same transverse projection as with the Sivers effect:
\begin{equation}
...
\end{equation}
\noindent Finally, when we consider the Worm-Gear effect, which measures the correlation between a quark spinning on a longitudinal axis and its trasnfers kick, we find:
\begin{equation}
...
All together, this means that CGD predicts that the ratios of these kicks are tightly linked to each other:
\begin{equation}
